...
def add_quarter_type(request):
    quarter_name = request.POST.get('quarter_name')
    if QuarterType.objects.filter(quarter_name=quarter_name).exists():
        messages.error(request, "Address Type Already Exists")
        return redirect('add_quarter_type')
    QuarterType.objects.create(quarter_name=quarter_name)
    messages.success(request, "Address Type Added Successfully")
    return redirect('add_quarter_type')
...
@csrf_protect
@user_passes_test(is_admin_or_operator)
def read_quarters(request):
    quarters = QuarterType.objects.all()
    return render(request, 'billing/quarter_type.html', {'quarters': quarters})


@login_required
@csrf_protect
@user_passes_test(is_admin_or_operator)
def edit_quarter_type(request):
    quarter_id = request.POST.get('quarter_id')
    quarter_name = request.POST.get('quarter_name')
    if QuarterType.objects.filter(quarter_name=quarter_name).exists():
        messages.error(request, "Address Type Already Exists")
        return redirect('read_types.html')
    QuarterType.objects.filter(quarter_id=quarter_id).update(
        quarter_name=quarter_name)
    messages.success(request, "Address Type Updated Successfully")
    return redirect('read_types.html')
...
@csrf_protect
@user_passes_test(is_admin_or_operator)
def delete_quarter_type(request):
    quarter_id = request.POST.get('quarter_id')
    if Room.objects.filter(quarter_type_id=quarter_id).exists():
        messages.error(request, "Quarter Type Has Rooms Associated With It")
        return redirect('read_types.html')
    QuarterType.objects.filter(quarter_id=quarter_id).delete()
    messages.success(request, "Quarter Type Deleted Successfully")
    return redirect('read_types.html')
...
def read_department(request):
    ...
    return render(request, 'billing/department.html', {'departments': departments})
...
def delete_department(request):
    department_id = request.POST.get('department_id')
    if UsersToDepartment.objects.filter(department_id=department_id).exists():
        messages.error(request, "Department Has Users Associated With It")
        return redirect('read_types.html')
...
def change_password(request):
    ...
    if request.method == 'POST':
        ...
        if not user.check_password(old_password):
            ...
            messages.error(
                request, "Your Old Password is Matching New Password.")
...
def index(request, message=None):
    ...
    for i in metered_bills_p:
        ...
        units_pre[i.month] += i.units_consumed
...
def calculate_bill(request):
    try:
        ...
        return redirect('fetch_bill.html')
    except Exception as e:
        messages.error(request, 'Error While Generating Bill')    
        return redirect('fetch_bill.html')
...
def get_bill(room_id, month, year, due_date, issue_date):
    try:
        ...
        if bill.get('duty_charges') != None:
            ...
            mult_factor = Meter.objects.get(
                meter_id=meter_id.meter_id).mult_factor
            ...
            MeteredBill.objects.create(user_name=username, full_name=full_name, email=email, user_department=user_department, opening_balance=opening_balance, room_number=room_number, quarter_type=quarter_type, sanctioned_load=sanctioned_load, meter_rate_name=meter_rate_name,
                                       meter_number=str(db_meter_to_room.meter_id), units_consumed=units_consumed, energy_charges=energy_charges, demand_charges=demand_charges, duty_charges=duty_charges, month=month, year=year, due_date=due_date, issued_date=issue_date, current_reading=sum_units_consumed + units_consumed, previous_reading=sum_units_consumed, mult_factor=mult_factor)
        else:
...
@csrf_protect
@user_passes_test(is_admin_or_operator)
def readTypes_page(request):
    context = {}
    try:
        quarterType_data = QuarterType.objects.all()
        department_data = Department.objects.all()
        context = {'segment': 'types',
                   'quarterTypes': quarterType_data,
                   'departments': department_data, }
        load_template = 'read_types.html'
        html_template = loader.get_template('home/' + load_template)
        return HttpResponse(html_template.render(context, request))
...
def delete_bill(request):
    if request.method == 'POST':
        try:
            ...
            return redirect('read_bills.html')

        except:
            messages.error(request, 'Bill not deleted')
            return redirect('read_bills.html')
...
def read_rooms(request, msg=None):  # also pass id
    ...
    try:
        ...
        messages.error(request, f"Error while getting address: {str(e)}")
...
@csrf_protect
@user_passes_test(is_admin_or_operator)
def manual_set_Reading_page(request, message=None):
    context = {}
    try:
        quarterType_data = QuarterType.objects.all()
        context = {
        ...
        if message is not None:
            context[message[0]] = message[1]
        load_template = 'manual_set_reading.html'
        html_template = loader.get_template('home/' + load_template)
        return HttpResponse(html_template.render(context, request))
...
@login_required(login_url='login.html')
@csrf_protect
def getBulkBill_page(request):

    context = {'segment':'readBills'}
    try:
        load_template = 'get_bulk_bill.html'
        html_template = loader.get_template('home/' + load_template)
        return HttpResponse(html_template.render(context, request))
...
def bulk_bill(request):
    ...
    if request.method == 'POST':
        ...
        metered_bills = MeteredBill.objects.filter(
            month=selected_month, year=selected_year)
        unmetered_bills = UnmeteredBill.objects.filter(
            month=selected_month, year=selected_year)
    ...
    return render(request, 'home/bulk_bills.html', context)
...
@user_passes_test(is_admin)
# for bulk bills pdf
def getBillpdf_page(request):

    context = {'segment':'readBills'}
    try:
        load_template = 'get_bills_pdf.html'
        html_template = loader.get_template('home/' + load_template)
        return HttpResponse(html_template.render(context, request))
...
def bills_pdf(request):
    ...
    return render(request, 'home/bills_pdf.html', context)
...
def createMeterRate_page(request):
    if request.method == "POST":
        try:
            ...
            if MeterRate.objects.filter(name=rate_name).exists():
                return HttpResponse('Rate Name already exists!')
            unit_range = []
            i = 0
            while True:
...
def createFlatRate_page(request):
    if request.method == "POST":
        try:
            req = request.POST
            rate_name = req.get('rate_name')
            if FlatRate.objects.filter(name=rate_name).exists():
                return HttpResponse('Rate Name already exists!')
            load_range = []
            i = 0
            while True:
    ...
    if request.method == "POST":
        try:
            ...
            if post_action == 'mr-delete':
                ...
                mrtr = MeterRateToRoom.objects.filter(
                    meter_rate_id=instance.id)
                if mrtr.exists():
                    messages.error(request, 'Error: Rooms with this Meter Rate exist!')
                ...
                if frtr.exists():
                    messages.error(request, 'Error: Rooms with this Flat Rate exist!')
                    return redirect(reverse('read_rates'))
                instance.delete()
            else:
                messages.error(request, 'Unknown Error!')
                return redirect(reverse('read_rates'))
            messages.success(request, "Rate Deleted Sucessfully")
        except:
            messages.error(request, 'Unknown Error!')
            return redirect(reverse('read_rates'))
    ...
    try:
        ...
        if room.is_metered:
            ...
            if meter_to_room:
                ...
                room.mult_factor = meter_to_room.meter_id.mult_factor
            meter_rate_to_room = MeterRateToRoom.objects.filter(
                room_id=room).first()
            if meter_rate_to_room:
                room.meter_rate_name = meter_rate_to_room.meter_rate_id.name
...
@csrf_protect
@user_passes_test(is_admin)
def updateReading_page(request, reading_id):
    context = {}
    try:
        currReading = Reading.objects.get(pk=reading_id)
        # room = Room.objects.get(pk=room_id)
        # quarterType_data = QuarterType.objects.all()
        # meterRate_data = MeterRate.objects.all()
        # flatRate_data = FlatRate.objects.all()
        month = currReading.month
        year = currReading.year
        meter_id = currReading.meter_id
        prev_reading = Reading.objects.filter(meter_id=meter_id, year__lte=year, month__lte=month).aggregate(
            Sum('units_consumed'))['units_consumed__sum'] + meter_id.initial_reading - currReading.units_consumed
        curr_reading = prev_reading + currReading.units_consumed
        reading_id = currReading.reading_id
        ...
        load_template = 'update_reading.html'
        html_template = loader.get_template('home/' + load_template)
        return HttpResponse(html_template.render(context, request))
...
def readReadings_page(request):
    ...
    try:
        ...
        for reading in readings_data:
            ...
            month = reading.month
            prev_reading = Reading.objects.filter(meter_id=meter_id, year__lte=year, month__lte=month).aggregate(
                Sum('units_consumed'))['units_consumed__sum'] + meter_id.initial_reading - reading.units_consumed
            curr_reading = prev_reading + reading.units_consumed
            if True:
                meter_to_room = MeterToRoom.objects.get(meter_id=meter_id)
                if meter_to_room:
    ...
    return render(request, 'home/read_readings.html', context)
...
def add_user(request):
    ...
    try:
        with transaction.atomic():
            ...
            # Get balance value as a string
            balance_str = request.POST.get('balance', '')
            if balance_str:  # Check if the value is not empty
            ...
            db_user = UserDetails.objects.create(
                email=email, full_name=fullname, user_type=usertype, balance=balance, django_user=django_user)
            ...
            subject = "Welcome to the Account Creation Billing System"
            message = f"Hello {fullname},\n\n"
            message += f"We are thrilled to welcome you to our Account Creation Billing System!\n\n"
            message += f"Your account with username '{username}' has been created successfully.\n"
            message += f"Please keep your temporary password, '{password}', confidential and ensure its security.\n\n"
            message += f"Thank you for choosing our system. We look forward to serving you.\n\n"
            message += f"Best regards,\n E Billing System Team ,\n NIT SRINAGAR"
...
def read_users(request, msg=None):
    ...
    try:
        ...
        user_to_rooms = UsersToRoom.objects.select_related(
            'room_id__quarter_type_id').filter(user_id__in=users)
        context = {
            'segment': 'readUsers',
            'users': users,
            'user_to_rooms': user_to_rooms,
        }
        if msg:
            context[msg[0]] = msg[1]
    except Exception as e:
...
def update_user(request):
    ...
    try:
        ...
        oldusername =user_auth.username
        if user_auth:
            user_auth.username = username
            newusername=user_auth.username
            
            user_auth.save()
        # user_db.username = username
        user_db.email = email
        newemail = user_db.email
        ...
        message = f"Hello {oldusername},\n\n"
        message += f"We want to inform you that your account has been updated in our system.\n"
        message += f"Your username remains as '{newusername}' and your password remains unchanged.\n\n"
        if oldemail != newemail:
            message += f"Your email address has been Changed to {newemail}.\n"
        message += f"Thank you for using our system. We're here to assist you.\n\n"
        message += f"Best regards,\nE Billing System Team,\nNIT SRINAGAR"
        send_mail(subject, message, EMAIL_HOST_USER, [oldemail, newemail])
...
def delete_user(request, user_id):
    ...
    try:
        ...
        if UsersToDepartment.objects.filter(user_id=user_id).exists():
            user_to_department = get_object_or_404(
                UsersToDepartment, user_id=user_id)
        ...
        subject = "Account Deletion from Billing System"
        message = f"Hello {user_fullname},\n\n"
        message += f"We regret to inform you that your account with username '{username}' has been deleted successfully from the Billing System.\n"
        message += f"If you believe this deletion was in error or have any questions, please reach out to our support team.\n\n"
        message += f"Thank you for your past engagement with our system.\n\n"
        message += f"Best regards,\n E Billing .\n NIT SRINAGAR"
        send_mail(subject, message, EMAIL_HOST_USER, [email])
    ...
    try:
        with transaction.atomic():
            ...
            if is_metered:
                ...
                mult_factor = request.POST.get('mult_factor')
                meter_rate_obj = MeterRate.objects.filter(
                    name=meter_rate_name).first()
                db_meter = Meter.objects.create(
                    initial_reading=initial_reading, mult_factor=mult_factor)
        ...
        messages.error(request, f"Error while creating Address")
    return createRoom_page(request)
    ...
    try:
        ...
        if is_metered:
            if room_db.is_metered:
                ...
                db_meter.mult_factor = mult_factor
                meter_rate_name = request.POST.get('meter_rate')
                meter_rate_obj = MeterRate.objects.filter(
                    name=meter_rate_name).first()
        ...
        messages.error(request, "Error while updating Address")
...
def update_reading(request):
    if request.method == 'POST':
        ...
        curr_reading = request.POST.get('curr_reading')
        db_reading = Reading.objects.get(reading_id=reading_id)
        month = db_reading.month
        year = db_reading.year
        meter_id = db_reading.meter_id.meter_id
        meter_to_room = MeterToRoom.objects.get(
            meter_id=meter_id).room_id.room_id
        db_room = Room.objects.get(room_id=meter_to_room)
        room_number = db_room.room_number
        quarter_type = QuarterType.objects.get(
            quarter_id=db_room.quarter_type_id.quarter_id)
        prev_reading = Reading.objects.filter(meter_id=meter_id, year__lte=year, month__lte=month).aggregate(Sum(
            'units_consumed'))['units_consumed__sum'] + db_reading.meter_id.initial_reading - db_reading.units_consumed
        units_consumed = float(curr_reading) - prev_reading

        if MeteredBill.objects.filter(month=month, year=year, room_number=room_number, quarter_type=quarter_type).exists() or units_consumed < 0:
            if MeteredBill.objects.filter(month=month, year=year, room_number=room_number, quarter_type=quarter_type).exists():
                messages.error(
                    request, "Metered bill already generated for this month.")
            else:
                messages.error(
                    request, "Units consumed cannot be negative.")
            context = {}
            context['reading'] = db_reading
            context['room_number'] = room_number
            context['quarter_type'] = quarter_type
            return render(request, f"home/update_reading.html", context)
        db_reading.units_consumed = units_consumed
        db_reading.save()
        messages.success(request, "Reading updated successfully.")
    result = readReadings_page(request)
    return result
...
def update_readings(request):
    if request.method == 'POST':
        ...
        try:
            ...
            # Collect the data to be updated in the database
            for index, row in df.iterrows():
                room_number = row['Room_Number']
                quarter_type = row['Quarter_Type']
                current_input_reading = float(row['Reading'])

                try:
                    quarter_type_instance = QuarterType.objects.get(
                        quarter_name=quarter_type)
                except QuarterType.DoesNotExist:
                    invalid_entries.append(
                        f'Quarter Type {quarter_type} does not exist in the database for Room {room_number}.')
                    continue

                room = Room.objects.filter(
                    quarter_type_id=quarter_type_instance, room_number=room_number).first()
                if room is None:
                    invalid_entries.append(
                        f'Room with the given room_number and quarter_type does not exist for Room {room_number}, {quarter_type}.')
                    continue

                meter_to_room = MeterToRoom.objects.filter(
                    room_id=room).first()
                if meter_to_room is None or meter_to_room.meter_id is None:
                    invalid_entries.append(
                        f'Meter ID not found for the given quarter number and quarter type for Room {room_number}, {quarter_type}.')
                    continue
                ...
                if meter is None:
                    invalid_entries.append(
                        f'Meter with the given meter_id does not exist for Room {room_number}, {quarter_type}.')
                    continue
                ...
                if reading_exists:
                    invalid_entries.append(
                        f'Reading already exists for Room {room_number}, {quarter_type}, {selected_month_name} {selected_year}.')
                    continue
                ...
                if latest_reading:
                    ...
                    if selected_year < latest_year or (selected_year == latest_year and selected_month < latest_month):
                        invalid_entries.append(
                            f'Cannot enter reading for a month earlier than the latest entry for Room {room_number}, {quarter_type}.')
                        continue
                ...
                new_reading = current_input_reading - \
                    (actual_initial_reading + total_previous_units)

                if new_reading < 0:
                    invalid_entries.append(
                        f'Invalid reading value for Room {room_number}, {quarter_type}. Please enter a valid value.')
                    continue
                # update here
                val = (meter, selected_month, selected_year)
                if val not in visited:
                    visited.add(val)
                    valid_entries.append(Reading(
                        meter_id=meter,
                        month=selected_month,
                        year=selected_year,
                        units_consumed=new_reading
                    ))
                    if not Reading.objects.filter(meter_id=meter, month=selected_month, year=selected_year).exists():
                        Reading.objects.create(
                            meter_id=meter, month=selected_month, year=selected_year, units_consumed=new_reading)
            # Generate a text file containing invalid entries
            # invalid_entries_file = 'invalid_entries.txt'
            # with open(invalid_entries_file, 'w') as file:
            #    for entry in invalid_entries:
            #        file.write(entry + '\n')

            # Return success message and link to download invalid entries file
            if invalid_entries:
                response_data = {
                    'status': 'error',
                    'message': 'An error occurred while processing the uploaded file. Please check the file format or select the correct month and year.',
                    'invalid_entries': invalid_entries,  # Add the invalid entries to the response
                }

                return JsonResponse(response_data)
                # success_message = 'Readings updated with some errors. Please check the invalid entries.'
                # download_link = f'<a href="{invalid_entries_file}" download>Download Invalid Entries</a>'
            else:
                success_message = 'Readings updated successfully from the uploaded file.'
                download_link = ''
            ...
            return JsonResponse({'status': 'error', 'message': 'Invalid file format. Please upload a valid Excel file.'})
        except Exception as e:
            return JsonResponse({'status': 'error', 'message': 'An error occurred while processing the uploaded file. Please check the file format and select the correct month and year.'})

    return render(request, 'update_readings.html')

# Manual-update-readings
...
@user_passes_test(is_admin_or_operator)
@transaction.atomic
def manual_update_readings(request):
    if request.method == 'POST':
        room_number = request.POST.get('room_number')
        quarter_type = request.POST.get('quarter_type')
        selected_month_name = request.POST.get('selected_month')
        ...
        quarter_type_instance = QuarterType.objects.get(
            quarter_name=quarter_type)
        room = Room.objects.get(
            quarter_type_id=quarter_type_instance, room_number=room_number)
